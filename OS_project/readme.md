...
Overall flow: Request received → pipe created → child process created using fork() → parent sends request through pipe → child reads request → file opened → read/write operation performed → file and pipe descriptors closed → child terminates → next request handled.
